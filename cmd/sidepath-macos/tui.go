...
func runTUI(ctx context.Context, cancel context.CancelFunc, errCh <-chan error, events <-chan uiEvent, node *blenode.Node, identity *core.Identity, current *blenode.Channel) error {
	...
	m.appendLines(true,
		fmt.Sprintf("Sidepath macOS  node=%s  phy=1m", identity.NodeID()),
...
func (m tuiModel) statusText() string {
	if !m.chatMode {
		return "Sidepath command"
	}
	if m.dmTarget != nil {
		return fmt.Sprintf("Sidepath DM %s", *m.dmTarget)
	}
	return fmt.Sprintf("Sidepath #%s", m.current.Name)
}

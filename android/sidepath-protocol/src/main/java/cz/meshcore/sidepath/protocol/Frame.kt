...
data class Frame(
    ...
    fun encode(): ByteArray {
        val buf = ByteBuffer.allocate(Sidepath.FRAME_HEADER_SIZE + data.size).order(ByteOrder.BIG_ENDIAN)
    ...
    companion object {
        fun decode(raw: ByteArray): Frame {
            require(raw.size >= Sidepath.FRAME_HEADER_SIZE) {
                "Frame too short: ${raw.size} bytes (need ${Sidepath.FRAME_HEADER_SIZE})"
            }
            val buf = ByteBuffer.wrap(raw).order(ByteOrder.BIG_ENDIAN)
            val version = buf.get().toInt() and 0xFF
            val transferId = ByteArray(Sidepath.TRANSFER_ID_BYTES).also { buf.get(it) }
            val fragmentIndex = buf.get().toInt() and 0xFF
            val fragmentCount = buf.get().toInt() and 0xFF
            val crc = buf.int
            val data = ByteArray(raw.size - Sidepath.FRAME_HEADER_SIZE).also { buf.get(it) }
            return Frame(version, transferId, fragmentIndex, fragmentCount, crc, data)
        }

        fun newTransferId(): ByteArray =
            ByteArray(Sidepath.TRANSFER_ID_BYTES).also { SecureRandom().nextBytes(it) }
        ...
        fun fragment(
            ...
            maxFrameSize: Int = Sidepath.MAX_FRAME_SIZE,
            transferId: ByteArray = newTransferId(),
        ): List<Frame> {
            require(transferId.size == Sidepath.TRANSFER_ID_BYTES) { "transfer id must be ${Sidepath.TRANSFER_ID_BYTES} bytes" }
            val maxData = (maxFrameSize - Sidepath.FRAME_HEADER_SIZE).coerceAtLeast(1)
            ...
            return chunks.mapIndexed { i, c ->
                Frame(Sidepath.FRAME_VERSION, transferId, i, count, crc, c)
            }
        }
    }
}
 ...
 */
class Reassembler(private val timeoutMs: Long = Sidepath.REASSEMBLY_TIMEOUT_MS) {

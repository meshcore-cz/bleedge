...
/**
 * NodeID is the compact 10-byte Sidepath routing address, derived from the first
...
class NodeId(val bytes: ByteArray) {
    init { require(bytes.size == Sidepath.NODE_ID_BYTES) { "NodeID must be ${Sidepath.NODE_ID_BYTES} bytes, got ${bytes.size}" } }
    ...
    companion object {
        val BROADCAST = NodeId(ByteArray(Sidepath.NODE_ID_BYTES))

        fun fromHex(hex: String): NodeId {
            require(hex.length == Sidepath.NODE_ID_BYTES * 2) {
                "NodeID hex must be ${Sidepath.NODE_ID_BYTES * 2} chars, got: $hex"
            }
            return NodeId(ByteArray(Sidepath.NODE_ID_BYTES) { hex.substring(it * 2, it * 2 + 2).toInt(16).toByte() })
        }

        fun random(): NodeId =
            NodeId(ByteArray(Sidepath.NODE_ID_BYTES).also { SecureRandom().nextBytes(it) })

        /** NodeID = public_key[0:10] (§3). */
        fun fromPublicKey(publicKey: ByteArray): NodeId {
            require(publicKey.size == Sidepath.PUBLIC_KEY_BYTES) { "public key must be ${Sidepath.PUBLIC_KEY_BYTES} bytes" }
            return NodeId(publicKey.copyOfRange(0, Sidepath.NODE_ID_BYTES))
        }
    }
}
